...
public class HttpProxyHandler extends BaseMessageHandler<HttpRequestMessage> {
        ...
        try {
            ...
            if (Strings.isNullOrEmpty(uri)) {
                HttpResponseMessage
                        ...
                        .sendRaw();
                Logs.HTTP.warn("receive bad request url is empty, message={}, dType={}", message, DetailTypes.HTTP_PROXY);
            }
            ...
            Logs.HTTP.error("send proxy request ex, message={}, request={}, error={}, dType={}", message, message, e.getMessage(), DetailTypes.HTTP_PROXY);
        } finally {
            Profiler.release();
        }
    }
    ...
    private static class DefaultHttpCallback implements HttpCallback {
        ...
        public void onResponse(HttpResponse httpResponse) {
            ...
            response.send();
            Logs.HTTP.info("send proxy request success end, message={} request={}, response={}, dType={}", request, request, response, DetailTypes.HTTP_PROXY);
        }
        ...
        public void onFailure(int statusCode, String reasonPhrase) {
            HttpResponseMessage
                    ...
                    .sendRaw();
            Logs.HTTP.warn("send proxy request failure end, message={} request={}, response={}, dType={}", request, request, statusCode + ":" + reasonPhrase, DetailTypes.HTTP_PROXY);
        }
        ...
        public void onException(Throwable throwable) {
            ...
            LOGGER.error("send proxy request ex end, message={} request={}, response={}, dType={}", request, request, 502, throwable, DetailTypes.HTTP_PROXY);
            Logs.HTTP.error("send proxy request ex end, message={} request={}, response={}, error={}, dType={}", request, request, 502, throwable.getMessage(), DetailTypes.HTTP_PROXY);
        }
        ...
        public void onTimeout() {
            HttpResponseMessage
                    ...
                    .sendRaw();
            Logs.HTTP.warn("send proxy request timeout end, message={} request={}, response={}, dType={}", request, request, 408, DetailTypes.HTTP_PROXY);
        }
